...
function template_main()
{
	...
	echo '
		<div id="calendar">
			<div id="month_grid" style="text-align: left;">
				<div class="cat_bar"><h3 class="catbg centertext" style="font-size: small;">Upcoming Events</h3></div>
				';
				
				//echo date("Y-m-d");
				$result = mysql_query("SELECT * FROM smf_calendar where end_date >= '" . date("Y-m-d") . "' ORDER BY start_date ASC");
        while($row = mysql_fetch_array($result)) {
					echo "<strong>";
					echo date("F j", strtotime($row['start_date']));
					if ($row['start_date'] != $row['end_date']) echo " - " . date("F j", strtotime($row['end_date']));
					echo "</strong><br>
					";
					if ($row['id_topic'] != "0") echo "<a href=\"index.php?topic=" . $row['id_topic'] . "\">";
					echo $row['title'];
					if (strlen($row['title']) == 60) echo "...";
					if ($row['id_topic'] != "0") echo "</a>";
